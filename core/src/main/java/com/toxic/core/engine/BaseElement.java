...
class BaseElement implements IElement {
    ...
    // Strelock : create smart caching of reusable cached images - why we should
    // it create again!
    // Strelock : provide smart hashCode() for wrapped Image object, to
    // determine if it is already existed
    // Strelock : maybe bound it with Resource manager and CachedImage object.
    // Strelock : enhanced animation model : add possibility launch animation with
    // Delay or even for signal
    // Strelock : instance of DRAWABLE_CANVAS : should be wrapped as separate
    // object
    // with additional functionally
    // Strelock : create smart fast hashCode for snapshot's produced objects,
    // shouldn't
    // create object, that keeping in memory, and has been already create...
    // Strelock : provide smart auto-caching engine
    // Strelock : investigate Workers performance benefits.
    // Strelock : provide sounds
    ...
    private void initTempSizeCanvas() {
        ...
        if (!isClipped()
            ...
            // should clean up and provided fall-back to GLContext
    ...
    // Strelock : call this method just during paint method, if necessary,
    ...
    void renderer() {
            ...
            if (this.bkground.getImage() != null && ((CachedImage) this.bkground.getImage()).getImage().isReady()) {
                ...
                if (this.bkground.isResize()) {
                    ...
                    if (((CachedImage) this.bkground.getImage()).getImage().width() < elemWidth
                        ...
                        if (isClipped()) {
                            DRAWABLE_CANVAS.canvas().drawImage(((CachedImage) this.bkground.getImage()).getImage(), 0,
                                0, elemWidth, elemHeight, 0, 0,
                                ((CachedImage) this.bkground.getImage()).getImage().width(),
                                ((CachedImage) this.bkground.getImage()).getImage().height());
                        }
                        else {
                            DRAWABLE_CANVAS.canvas().drawImage(((CachedImage) this.bkground.getImage()).getImage(), 0,
                                0, DRAWABLE_CANVAS.width(), DRAWABLE_CANVAS.height());
                        }
                    }
                }
            }
        ...
        if (this.text != null && !this.text.isEmpty() && this.textFormat != null) {
            ...
            if (!((TextFormat) this.textFormat).isMargin()) {
                if (this.textFormatInner != null) {
                    ...
                    TextLayout lauoyt = PlayN.graphics().layoutText(this.text,
                        ((TextFormat) this.textFormat).getFormat());
                ...
                if (this.textFormatInner != null) {
                    ...
                    TextLayout lauoyt = PlayN.graphics().layoutText(this.text, this.textFormatInner);
                    DRAWABLE_CANVAS.canvas().fillText(lauoyt, this.textFormat.getMarginLeft(),
                        this.textFormat.getMarginTop());
        ...
        if (this.debug) {
            ...
            DRAWABLE_CANVAS.canvas().drawLine(BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH,
                BaseElement.DRAWABLE_CANVAS.height() - 1 - LINE_WIDTH,
                BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH, LINE_WIDTH);
            DRAWABLE_CANVAS.canvas().drawLine(BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH, LINE_WIDTH,
    ...
    void dispatchMoveEvent(final ActionEvent e) {
        ...
        // Strelock : this bug of uncontrolled out of boundary events
        this.previousEvent = this.currentEvent;
        this.currentEvent = e;
        // Strelock : here is mistake.
        if (this.moveHandler != null
            && new Point(BaseElement.this.previousEvent.getX(), BaseElement.this.previousEvent.getY()).distance(
                e.getX(), e.getY()) > EventManager.RADIUS_HIT) {
    ...
    void dispatchEventStart(final ActionEvent event) {
        ...
        if (this.holdHandler != null) {
            ...
            // Strelock : this parameter should be passed from holder handler
                ...
                EventManager.TIME_EVERY_HOLD, new Runnable() {
                    ...
                    public void run() {
                        if (BaseElement.this.currentEvent != null
                            && (new Point(BaseElement.this.currentEvent.getX(), BaseElement.this.currentEvent.getY())
                                .distance(event.getX(), event.getY()) < EventManager.RADIUS_HIT)) {
    ...
    public String toString() {
        return "The element id = " + this.id + ", position :[" + this.layer.tx() + ":" + this.layer.ty()
            + "], size : [" + width() + ":" + height() + "]";
    }
    ...
    public final CancelHandler animateShake(final float amplitudeX, final float amplitudeY, final Interpolator mode,
        ...
        if (mode == null) {
            PlayN.log()
                .error("You have tried animate element : " + this.toString() + "\n" + "with empty Interpolator.");
    ...
    public final CancelHandler animateTransition(final float x, final float y, final Interpolator mode,
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim
                .tweenTranslation(BaseElement.this.layer).in(duration)
                .from(BaseElement.this.layer.tx(), BaseElement.this.layer.ty()).to(x, y).using(mode).then()
                .action(new Runnable() {
    ...
    public final CancelHandler animateAction(final CustomAnimation action, final Interpolator mode, final int duration) {
        ...
        if (action == null) {
            ...
            throw new IllegalArgumentException("You have tried animate empty CustomAction on element : "
                + this.toString());
        }
        if (duration <= 0) {
    ...
    public final CancelHandler animateRotate(final float angle, final Interpolator mode, final int duration) {
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim.tweenRotation(BaseElement.this.layer)
                .in(duration).from(BaseElement.this.layer.rotation()).to(BaseElement.this.layer.rotation() + angle)
                .using(mode).then().action(new Runnable() {
