...
class BaseElement
    ...
    private void initTempSizeCanvas() {
        ...
        if (!isClipped()
            ...
            && (DRAWABLE_CANVAS.width() != ((CachedImage) this.bkground.getImage()).getImage().width() || DRAWABLE_CANVAS.height() != ((CachedImage) this.bkground.getImage()).getImage()
                                                                                                                                                                              .height())) {
    ...
    void renderer() {
        ...
        if (this.bkground != null) {
            ...
            if (this.bkground.getImage() != null && ((CachedImage) this.bkground.getImage()).getImage().isReady()) {
                ...
                if (this.bkground.isResize()) {
                    DRAWABLE_CANVAS.canvas().drawImage(((CachedImage) this.bkground.getImage()).getImage(), 0, 0, elemWidth, elemHeight);
                } else {
                    ((CachedImage) this.bkground.getImage()).getImage().setRepeat(this.bkground.isRepeatX(), this.bkground.isRepeatY());
                    if (((CachedImage) this.bkground.getImage()).getImage().width() < elemWidth
                        ...
                        DRAWABLE_CANVAS.canvas().drawImage(((CachedImage) this.bkground.getImage()).getImage(),
                                                           0,
                                                           0,
            ...
            if (!((TextFormat) this.textFormat).isMargin()) {
                if (this.textFormatInner != null) {
                    ...
                    switch (this.textFormatInner.align) {
                        ...
                        case CENTER:
                            DRAWABLE_CANVAS.canvas().fillText(lauoyt, (elemWidth - lauoyt.width()) / 2f, (elemHeight - lauoyt.height()) / 2f);
                            break;

                        case RIGHT:
                            DRAWABLE_CANVAS.canvas().fillText(lauoyt, (elemWidth - lauoyt.width()), (elemHeight - lauoyt.height()) / 2f);
                            break;
        ...
        if (this.debug) {
            ...
            DRAWABLE_CANVAS.canvas().drawLine(LINE_WIDTH, LINE_WIDTH, LINE_WIDTH, BaseElement.DRAWABLE_CANVAS.height() - 1 - LINE_WIDTH);
            DRAWABLE_CANVAS.canvas().drawLine(LINE_WIDTH,
                                              BaseElement.DRAWABLE_CANVAS.height() - 1 - LINE_WIDTH,
                                              BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH,
                                              BaseElement.DRAWABLE_CANVAS.height() - 1 - LINE_WIDTH);
            DRAWABLE_CANVAS.canvas().drawLine(BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH,
                                              BaseElement.DRAWABLE_CANVAS.height() - 1 - LINE_WIDTH,
                                              BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH,
                                              LINE_WIDTH);
            DRAWABLE_CANVAS.canvas().drawLine(BaseElement.DRAWABLE_CANVAS.width() - 1 - LINE_WIDTH, LINE_WIDTH, LINE_WIDTH, LINE_WIDTH);
        }
        initImageLayer();
        if (this.canvasInit) {
    ...
    void dispatchSelectEvent(final ActionEvent e) {
        ...
        if (this.moveHandler == null
            ...
            || (new Point(this.initialEvent.getX(), this.initialEvent.getY()).distance(this.currentEvent.getX(), this.currentEvent.getY()) < EventManager.RADIUS_HIT)) {
            ...
            this.holdCancel = TimerUtility.getInstance().atThenEvery(EventManager.TIME_TO_NOTIFY_HOLD, EventManager.TIME_EVERY_HOLD, new Runnable() {
                ...
                public void run() {
                    if (BaseElement.this.currentEvent != null
                        && (new Point(BaseElement.this.currentEvent.getX(), BaseElement.this.currentEvent.getY()).distance(event.getX(), event.getY()) < EventManager.RADIUS_HIT)) {
    ...
    private void addOnRightPlace(BaseElement child) {
        ...
        while (!this.children.isEmpty() && this.children.size() > index && this.children.get(index).depth() >= child.depth()) {
            index++;
        }
    ...
    public String toString() {
        return "The element id = " + this.id + ", position :[" + this.layer.tx() + ":" + this.layer.ty() + "], size : [" + width() + ":" + height() + "]";
    }
    ...
    @Override
    public final CancelHandler animateShake(final float amplitudeX, final float amplitudeY, final Interpolator mode, final float duration) {
        if (mode == null) {
        ...
        if (duration <= 0) {
            PlayN.log().error("You have set negative time to animation on element : " + this.toString() + "\n with value : " + duration);
            throw new IllegalStateException("You have set negative time to animation on element : " + this.toString() + "\n with value : " + duration);
        }
        if (amplitudeX < 0 || amplitudeY < 0) {
            PlayN.log().error("You have set negative amplitudes value to animation on element : " + this.toString() + "\n with value : [" + amplitudeX + ":"
                              + amplitudeY + "].");
            throw new IllegalStateException("You have set negative amplitudes value to animation on element : " + this.toString() + "\n with value : ["
                                            + amplitudeX + ":" + amplitudeY + "].");
        }
        if (this.animStoppers.get(AnimationType.SHAKE) != null) {
            PlayN.log().warn("Previous shake animation with element : " + this.toString() + "Doesn't yet finished.\nWill be forced to stop now.");
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim.shake(BaseElement.this.layer)
                                                                               .bounds(-amplitudeX, amplitudeX, -amplitudeY, amplitudeY)
                                                                               .using(mode)
                                                                               .in(duration)
                                                                               .then()
                                                                               .action(new Runnable() {
    ...
    @Override
    public final CancelHandler animateTransition(final float x, final float y, final Interpolator mode, final float duration) {
        if (mode == null) {
            PlayN.log().error("You have tried animate element : " + this.toString() + "\n" + "with empty Interpolator.");
            throw new IllegalArgumentException("You have tried animate element : " + this.toString() + "\n" + "with empty Interpolator.");
        }
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim.tweenTranslation(BaseElement.this.layer)
                                                                               .in(duration)
                                                                               .from(BaseElement.this.layer.tx(), BaseElement.this.layer.ty())
                                                                               .to(x, y)
                                                                               .using(mode)
                                                                               .then()
                                                                               .action(new Runnable() {
    ...
    public final CancelHandler animateAction(final CustomAnimation action, final Interpolator mode, final int duration) {
        ...
        if (this.animStoppers.get(AnimationType.CUSTOM) != null) {
            log.warn("Previous shake animation with element : " + this.toString() + "Doesn't yet finished.\nWill be forced to stop now.");
    ...
    public final CancelHandler animateRotate(final float angle, final Interpolator mode, final int duration) {
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim.tweenRotation(BaseElement.this.layer)
                                                                               .in(duration)
                                                                               .from(BaseElement.this.layer.rotation())
                                                                               .to(BaseElement.this.layer.rotation() + angle)
                                                                               .using(mode)
                                                                               .then()
                                                                               .action(new Runnable() {
    ...
    public final CancelHandler animateOpacity(final float to, final Interpolator mode, final int duration) {
        ...
        final CancelHandler cancel = new CancelHandler() {
            ...
            private final tripleplay.anim.Animation.Handle cancelHandler = anim.tweenAlpha(BaseElement.this.layer)
                                                                               .in(duration)
                                                                               .from(BaseElement.this.layer.alpha())
                                                                               .to(to)
                                                                               .using(mode)
                                                                               .then()
                                                                               .action(new Runnable() {
    ...
    /**
     * <p>
     * Used for handy difference of animation {@link tripleplay.anim.Animation.Handle}.
     * </p>
     * <br/>
     * 
     * @author Strelock
    ...
    public void setTextFromat(ITextFormat format) {
        if (((TextFormat) format).getFormat().shouldWrap() || ((TextFormat) format).getFormat().wrapWidth > width()) {
            this.textFormatInner = new playn.core.TextFormat(((TextFormat) format).getFormat().font, height(), ((TextFormat) format).getFormat().align);
        } else {
    ...
    public boolean equals(Object obj) {
        ...
        if (elem.parent == this.parent && elem.size.equals(this.size) && elem.alpha() == this.alpha() && elem.propogative == this.propogative
            && elem.selectHandler == this.selectHandler && elem.moveHandler == this.moveHandler && elem.id.equals(this.id) && elem.layer.equals(this.layer)) {
            return true;
        }
